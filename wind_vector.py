...
def draw_compass(vector1, vector2, result_vector):
    compass_data = {
        "Direction": ["Vector 1", "Vector 2", "Resultant"],
        "Angle (degrees)": [vector1[0], vector2[0], result_vector[0]],
        "Magnitude": [vector1[1], vector2[1], result_vector[1]]
    }
    fig = px.bar_polar(compass_data, r="Magnitude", theta="Angle (degrees)", color="Direction",
                       title="Compass Diagram", range_theta=[0, 360])
    st.plotly_chart(fig)
...
if st.button("Add Vectors"):
    ...
    result_vector = compass_vector_addition(vector1, vector2)

    result_angle, result_magnitude, wind_angle_deg = compass_vector_addition(vector1, vector2)

    draw_compass(vector1, vector2, result_vector)
